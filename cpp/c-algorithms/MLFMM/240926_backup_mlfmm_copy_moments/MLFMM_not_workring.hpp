...
template <typename Monitor>
 ...
 public:
  ...
  Eigen::MatrixXd compute_A(int max_tree_lvl = 3,int L = 9, int min_numel = 1, bool compute_src_moments=true){

    // allocate space for A
    Eigen::MatrixXd A(num_tar_, 3);
    A.setZero();

    if (compute_src_moments){
  ...
  void compute_source_moments(int max_tree_lvl = 3,int L = 9, int min_numel = 1){
  ...
  void compute_local_moments(int max_tree_lvl = 3,int L = 9, int min_numel = 1){
  ...
  void add_source_moments(ClusterTree *tree_ext, double factor){

    // get a target tree memory pointer
    std::shared_ptr<ClusterTreeMemory> tree_mem = src_tree_.get_tree_memory();

    // get a extern tree memory pointer
    std::shared_ptr<ClusterTreeMemory> ext_tree_mem = tree_ext->get_tree_memory();

    // set the level to the maximum
    int l = max_tree_lvl_ + 1;

    // this integer points to the first cell on the maximum level
    int first_cell_id = tree_mem->cumNumElements(l);
    
    // a target cell counter
    int cnt = 0;

    // make an iterator for the external memory
    std::vector<ClusterTreeNode>::iterator ext_src_it = ext_tree_mem->lbegin(l);

    // iterate over the target cells
    for(std::vector<ClusterTreeNode>::iterator src_it = tree_mem->lbegin(l); src_it < tree_mem->lend(l) ; src_it++){
      
      // add the locals
      src_it->moments_ += factor*ext_src_it[cnt].moments_;
  ...
  void add_far_field(ClusterTree *tree_ext, double factor){

    // get a target tree memory pointer
    std::shared_ptr<ClusterTreeMemory> tree_mem = tar_tree_.get_tree_memory();
    ...
    // make an iterator for the external memory
    std::vector<ClusterTreeNode>::iterator ext_tar_it = ext_tree_mem->lbegin(l);

    // iterate over the target cells
    for(std::vector<ClusterTreeNode>::iterator tar_it = tree_mem->lbegin(l); tar_it < tree_mem->lend(l) ; tar_it++){
      
      // add the locals
      tar_it->locals_ += factor*ext_tar_it[cnt].locals_;
  ...
  ClusterTree *get_tar_tree(){
    
    return &tar_tree_;
    
  } 

  ClusterTree *get_src_tree(){
  ...
  Eigen::MatrixXd evaluate_far_field(){
      ...
      #pragma omp critical
      {
        //std::cout << "my_result = " << my_result.transpose() << std::endl;
...
private:
  ...
  void compute_leaf_moments(){
    ...
    #pragma omp parallel
    {
      ...
      // get an iterator. On linux, we could define the parallel loop over this iterator. But
      // on windows this does not work
      // std::cout << "get the source cell iterator" << std::endl;
      std::vector<ClusterTreeNode>::iterator  src_it = src_tree_mem->lbegin(l);
      
      // the number of source cells on this level
      int num_src_cells = std::distance(src_tree_mem->lbegin(l), src_tree_mem->lend(l));
      ...
      for(int i = 0; i < num_src_cells; i++)
      {
        ...
        if(src_it[i].sons_.size() == 0){

          //allocate the multipole moments
          Eigen::MatrixXcd my_moments(num_coeffs_, 3);
          my_moments.setZero();
          
          //loop over all indices to sum up the contributions
          for (int j : src_it[i].indices_){

            // We need something like this
            monitor_.computeMoments(&my_moments, L_, src_.row(j).transpose() - src_it[i].center_, src_vec_.row(j));
